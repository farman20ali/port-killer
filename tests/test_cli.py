...
def test_safety_policy_custom_config():
    ...
    args = argparse.Namespace(
        bypass_safety=False, protected_ports=[8080], protected_processes=["custom-app"]
    )
...
def test_confirm_docker_rm(monkeypatch, capsys):
    ...
    assert (
        confirm_docker_rm(
            "my-container", "1234567890123456", assume_yes=True, force=True
        )
        is True
    )
    ...
    assert (
        confirm_docker_rm(
            "my-container", "1234567890123456", assume_yes=True, force=False
        )
        is False
    )
